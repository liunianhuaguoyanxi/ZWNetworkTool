...
// Added by Kiwi - begin
@property (strong, nonatomic) NSTimer * pingWebSocketTimer;
// Added by Kiwi - end
...
- (void)disconnect
{
    ...
    // Added by Kiwi - begin
    if (self.pingWebSocketTimer.isValid) {
        [self.pingWebSocketTimer invalidate];
        self.pingWebSocketTimer = nil;
    }
    // Added by Kiwi - end
}
...
- (void)sendBayeuxHandshake
{
    ...
    NSDictionary *message = @{@"channel":                  @"/meta/handshake",
                              ...
                              @"user": @{@"id": [NSString stringWithFormat:@"%lld", self.ewei_userID],
                                         @"providerId": [NSString stringWithFormat:@"%lld", self.ewei_providerID],
                                         @"platform": @"ios_console"
                                         }};
...
- (void)connectWebSocket
{
    ...
    [mRequest setValue:self.orginHeader forHTTPHeaderField:@"Origin"];
...
{
    // Added by Kiwi - begin
    self.pingWebSocketTimer = [NSTimer scheduledTimerWithTimeInterval:self.pingInterval target:self selector:@selector(pingWebSocket) userInfo:nil repeats:NO];
    /* - Old code
...
- (void)pingWebSocket
{
    if (self.isConnected) {
        ...
        LOG(@"Sending ping...");
//        [self.webSocket send:@"[]"];
        [self.webSocket sendString:@"[]" error:nil];
        [self setupPingTimer];
    }
}
                ...
                if ([channel isEqualToString:@"/meta/handshake"]) {
                    ...
                    if ([message[@"successful"] boolValue]) {
                        if ([self.delegate respondsToSelector:@selector(bayeuxClient:subscribedToChannel:)])
                            [self.delegate bayeuxClient:self subscribedToChannel:message[@"subscription"]];
                    } else {
